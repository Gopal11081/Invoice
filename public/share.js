...
function renderInvoice(inv) {
  ...
  const invNum = inv.invoice_number || 'INV-0001';
  ...
  preview.innerHTML = `
    ...
    ${seller.bank_name || seller.account_number ? `<div class="inv-bank-section"><div><div class="inv-bank-title">Bank Details</div><div class="inv-bank-info">${seller.bank_name ? '<strong>Bank:</strong> ' + escapeHtml(seller.bank_name) + '<br>' : ''}${seller.account_number ? '<strong>A/C:</strong> ' + escapeHtml(seller.account_number) + '<br>' : ''}${seller.ifsc_code ? '<strong>IFSC:</strong> ' + escapeHtml(seller.ifsc_code) + '<br>' : ''}${seller.bank_branch ? '<strong>Branch:</strong> ' + escapeHtml(seller.bank_branch) : ''}</div></div><div><div class="inv-terms-title">Terms & Conditions</div><div class="inv-terms-text">${escapeHtml(terms)}</div></div></div>` : ''}
