...
export async function getInvoices(limitVal = 20, startAfter = null) {
  ...
  try {
    ...
    if (startAfter) {
      if (typeof startAfter === 'string' && startAfter.includes('_')) {
        const [invoiceDate, createdAt] = startAfter.split('_');
        query = query.startAfter(invoiceDate, createdAt);
      } else {
    ...
    snapshot.forEach(doc => {
      const data = doc.data();
      // Recalculate grand_total from items if it's missing or 0 (fixes existing invoices affected by schema bug)
      let grandTotal = data.grand_total || 0;
      if (!grandTotal && data.items && data.items.length > 0) {
        let total = 0;
...
export async function getDetailedInvoices() {
  ...
  try {
    ...
    // Recalculate financial fields for invoices where grand_total is 0 (fixes existing data affected by schema bug)
    list.forEach(inv => {
